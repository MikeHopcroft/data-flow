...
* It may be beneficial to disallow the calling of functions not on an approved list.
* It may be beneficial to disallow the storing of unapproved function in object properties. (these functions could be unwittingly called by the host, say, during JSON serialization). Note that a safer solution would disallow the use of any value that had unapproved functions anywhere in its tree. Not sure whether this would be feasible since most objects have non-owned properties that include functions.
* It may be beneficial to avoid copying POJOs dervived from LLM prompts. Copying increases the risk of prototype poisoning and overwriting other functions.
* It may be beneficial to threat model the risk of leaking secrets into responses sent back to the end user.
