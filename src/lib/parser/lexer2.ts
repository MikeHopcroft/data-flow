...
class TokenImpl2<T> implements Token<T> {
  ...
  public retokenize(mode: number): TokenImpl2<T> | undefined {
    console.log(
      `retokenize ${this.pos.index}: "${this.text}" at "${this.input.slice(
        this.pos.index
      )}"`
    );
    try {
...
export function buildLexer2<T>(modes: [boolean, RegExp, T][][]): Lexer<T> {
...
export function templateTok<T>(toMatch: T): Parser<T, Token<T>> {
  return {
    parse(token: Token<T> | undefined): ParserOutput<T, Token<T>> {
      console.log('---------------------------');
      console.log(`parse ${toMatch}`);
      // console.log(`parse ${tokenNames[toMatch as number]}`);
